...
router.get('/:userId/public', [
  ...
  try {
    ...
    const user = await User.findById(userId)
      .select('username fullName avatar bio location statistics createdAt lastLogin');
...
router.get('/search', [
  ...
  try {
    ...
    const [users, total] = await Promise.all([
      User.find(filters)
        .select('username fullName avatar bio statistics.averageRating statistics.totalTrades')
